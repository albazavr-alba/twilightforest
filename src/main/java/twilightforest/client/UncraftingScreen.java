	...
	protected void extractTooltip(GuiGraphicsExtractor graphics, int pX, int pY) {
		...
		for (int i = 0; i < 9; i++) {
			if (container.getCarried().isEmpty()
				&& container.slots.get(2 + i).hasItem()
				&& this.hoveredSlot == container.slots.get(11 + i)
				&& !container.slots.get(11 + i).hasItem()) {

				ItemStack itemStack = container.slots.get(2 + i).getItem();
				List<ClientTooltipComponent> lines = getTooltipFromItem(minecraft, itemStack).stream().map(Component::getVisualOrderText).map(ClientTooltipComponent::create).toList();

				graphics.tooltip(this.font, lines, pX, pY, positioner, null);
				return;
			}
		}
		...
		if (container.slots.getFirst().hasItem()
			...
			Component alertText = Component.translatable("container.twilightforest.uncrafting_table.disabled_item").withStyle(ChatFormatting.RED);
			List<ClientTooltipComponent> lines = List.of(ClientTooltipComponent.create(alertText.getVisualOrderText()));
