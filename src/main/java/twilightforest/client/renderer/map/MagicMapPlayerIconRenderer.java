...
public class MagicMapPlayerIconRenderer implements IMapDecorationRenderer {
	...
	@Override
	public boolean render(MapRenderState.MapDecorationRenderState decorationRenderState, PoseStack stack, @NotNull SubmitNodeCollector submitNodeCollector, @NotNull MapRenderState mapRenderState, @NotNull TextureAtlas decorationSprites, boolean inItemFrame, int packedLight, int index) {
		stack.pushPose();
		...
		if (textureatlassprite == null) return false;
		...
		submitNodeCollector.submitCustomGeometry(stack, renderType, (_, consumer) -> {
			consumer.addVertex(matrix4f1, -1.0F, 1.0F, -0.3F).setColor(0xFFFFFFFF).setUv(f2, f3).setLight(packedLight);
			consumer.addVertex(matrix4f1, 1.0F, 1.0F, -0.3F).setColor(0xFFFFFFFF).setUv(f4, f3).setLight(packedLight);
			consumer.addVertex(matrix4f1, 1.0F, -1.0F, -0.3F).setColor(0xFFFFFFFF).setUv(f4, f5).setLight(packedLight);
			consumer.addVertex(matrix4f1, -1.0F, -1.0F, -0.3F).setColor(0xFFFFFFFF).setUv(f2, f5).setLight(packedLight);
		});
