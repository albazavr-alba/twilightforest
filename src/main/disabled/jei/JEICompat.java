...
public class JEICompat implements IModPlugin {
	...
	public static final IIngredientType<@NotNull FakeEntityType> ENTITY_TYPE = () -> FakeEntityType.class;
	public static final IIngredientType<@NotNull FakeItemEntity> FAKE_ITEM_ENTITY = () -> FakeItemEntity.class;
	...
	public void registerCategories(IRecipeCategoryRegistration registration) {
		...
		RecipeManager manager = Objects.requireNonNull(Minecraft.getInstance().level).getServer().getRecipeManager();
	}
	...
	public void registerRecipes(IRecipeRegistration registration) {
		...
		RecipeManager manager = Objects.requireNonNull(Minecraft.getInstance().level).getServer().getRecipeManager();
		if (!TFConfig.disableEntireTable) {
			List<RecipeHolder<? extends @NotNull CraftingRecipe>> recipes = RecipeViewerConstants.getAllUncraftingRecipes(manager);
		...
		registration.addRecipes(DryingCategory.DRYING, manager.recipeMap().byType(TFRecipes.DRYING_RECIPE.get()).stream().filter(holder -> !holder.value().getResult().is(TFItems.STALE_BREAD)).map(RecipeHolder::value).toList());
